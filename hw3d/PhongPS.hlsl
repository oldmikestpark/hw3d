...
float4 main(float3 worldPos : Position, float3 n : Normal, float2 tc : Texcood) : SV_Target
{
    ...
    const float3 specular = att * (diffuseColor * diffuseIntensity) * specularIntensity * pow(saturate(dot(normalize(r), normalize(-worldPos))), specularPower);
    // final color
