...
Graphics::Graphics(HWND hWnd)
{
	...
	GFX_THROW_INFO(pDevice->CreateTexture2D(&descDepth, nullptr, &pDepthStensil));
	...
	GFX_THROW_INFO(pDevice->CreateDepthStencilView(pDepthStensil.Get(), &dsvDesc, &pDSV));
...
void Graphics::DrawTestTriangle(float angle, float x, float z)
{
	...
	pContext->IASetVertexBuffers(0u, 1u, pVertexBuffer.GetAddressOf(), &strid, &offset);
	...
	// lookup table for cube face color
...
const char* Graphics::HrException::what() const noexcept
{
	...
	oss << GetType() << std::endl
		...
		<< "[Error Code] " << GetErrorString() << std::endl
